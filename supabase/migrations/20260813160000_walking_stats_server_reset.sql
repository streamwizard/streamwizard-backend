-- Walking-stats v5: hand distance resets to the server.
--
-- v4 zeroed the banked total client-side whenever the banked stream id
-- differed from sys.stream_id at load. That compare has no crash guard: a
-- dropped IRL stream restarted five minutes later carries a NEW stream id,
-- and the widget wiped the day's kilometres on its next load. The server's
-- reset-on-stream policy (user_state_definitions, migration
-- 20260813120000) does the same reset with a grace window, whether or not
-- any overlay is connected — so the widget now restores unconditionally and
-- adopts resets pushed to it as streamwizard.user_state frames.
--
-- The 'stream.online' listener v4 kept "because it costs nothing" is
-- removed: it never fired. That event only reaches the rest-api webhook,
-- not an overlay's room. Its replacement (the user_state frames) actually
-- arrives, and also keeps st.streamId current mid-walk so flushes bank
-- against the running stream instead of the one at page load.
--
-- Also seeds the 'distance' definition (policy stream, reset {"m": 0},
-- grace 900 s) for every user holding a walking-stats snapshot; without the
-- row the server never resets and v5 has no client fallback by design.
-- Follow-up: seed it at widget placement for future users.
--
-- Snapshot copies in overlay_widgets are updated by md5 match against the
-- v4 hash, same as previous walking-stats migrations: snapshots have no
-- template linkage, so a template-only update would leave existing users on
-- old code.

DO $$
DECLARE
  v_prev_md5 constant text := '42d1d6516f4b541235211413b5360c2d';
  v_snapshots integer;
...
(() => {
  ...
  const st = {
    ...
    /* streamId:    the stream the banked total belongs to (see loadState).
       lastWritten: total at the last successful save — the flush is triggered
                    by metres travelled, not by a timer, so a parked phone
                    costs nothing. */
    streamId: null, lastWritten: 0, lastFactorWritten: null,
  ...
  /* ---------- persistence ---------- */
  /* Distance lives in CHANNEL state (StreamWizard.userState), not this
     widget's private blob, because the moment that matters happens while
     nobody is connected. A widget only receives stream.online if it is open
     when it fires, so "reset on stream.online" silently does nothing for a
     streamer who goes live before starting OBS — and the widget then restores
     the last walk's kilometres as if they belonged to this stream.

     Channel state fixes that by inverting the question. The server records
     which stream is current whether or not anyone is listening; the widget
     banks its total ALONGSIDE the stream it belongs to and compares the two
     at load. Missing an event stops mattering, and so do reloads and OBS
     restarts. */
  const KEY_DISTANCE = 'distance';        /* { m, streamId } */
  const KEY_CAL_FACTOR = 'gps_cal_factor';
  /* Metres of travel between writes. A timer keeps writing while a phone sits
     on a table; triggering on distance means a standstill costs nothing and a
     walk costs roughly one write every 7 s. */
  const FLUSH_EVERY_M = 10;

  function hasState() {
    return !!(window.StreamWizard && StreamWizard.userState);
  }
  async function loadState() {
    if (!hasState()) return;
    try {
      /* One request for every key rather than one per key: this runs on the
         load path, in front of the first render. */
      const saved = await StreamWizard.userState.getAll();
      ...
      /* The calibration factor is a property of the DEVICE, not the walk —
         ...
         starts already corrected instead of re-learning for minutes. It has
         its own key for the same reason: anything that zeroes the distance
         must not be able to take the factor with it. */
      const factor = Number(saved[KEY_CAL_FACTOR]);
      if (Number.isFinite(factor)) {
        st.cal.factor = clamp(factor, 0.9, 1.25);
        st.cal.savedFactor = st.cal.factor;
        st.lastFactorWritten = st.cal.factor;
      }

      const current = saved['sys.stream_id'] || null;
      st.streamId = current;

      const banked = saved[KEY_DISTANCE];
      const total = banked && Number(banked.m);
      const mode = txt(cfg.distanceReset, 'stream');
      if (mode === 'load' || !Number.isFinite(total)) return;

      /* v5: reset-on-stream is the SERVER's job now (user_state_definitions,
         policy 'stream'). It zeroes the banked value at stream.online whether
         or not anything is connected, and its crash guard keeps the total
         when a dropped stream comes back inside the grace window — a case
         the old client-side "different stream id → zero" compare wiped.
         So the widget restores whatever is banked, unconditionally. */
      st.total = total;
      st.lastWritten = total;
    } catch (err) {
  ...
  /* The factor is only banked once THIS session has proven it (three
     ...
     previously trusted value, or a neutral 1: a factor half-learned from one
     noisy window must not become every future stream's starting point. */
  function factorToSave() {
    return st.cal.trusted
      ? st.cal.factor
  ...
  function flushState() {
    ...
    st.dirty = false;
    st.lastWritten = st.total;

    /* Distance and the stream it belongs to are one value on purpose: written
       separately they could disagree, and a total carrying the wrong stream
       id is exactly the bug this replaces. */
    Promise.resolve()
      .then(() => StreamWizard.userState.set(KEY_DISTANCE, {
        m: Math.round(st.total),
        streamId: st.streamId,
        savedAt: Date.now()
      }))
      .catch(() => {});

    /* Written only when it actually moves. The factor converges early and
       then sits still, so re-sending it every 10 m would double this
       widget's request rate for nothing. */
    const factor = Math.round(factorToSave() * 1000) / 1000;
    if (factor !== st.lastFactorWritten) {
      st.lastFactorWritten = factor;
      Promise.resolve()
        .then(() => StreamWizard.userState.set(KEY_CAL_FACTOR, factor))
        .catch(() => {});
    }
  }
  function maybeFlush() {
    if (!st.dirty) return;
    if (st.total - st.lastWritten < FLUSH_EVERY_M) return;
    flushState();
  }
  function startAutoSave() {
    /* The last stretch is worth more than the periodic ones: closing OBS at
       the end of a walk would otherwise drop up to 10 m, every time. */
    addEventListener('pagehide', () => { if (st.dirty) flushState(); });
    addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden' && st.dirty) flushState();
    });
  }
  function resetDistance(streamId) {
    /* Distance resets; the learned calibration factor survives on purpose. */
    st.total = 0;
    /* lastWritten has to come back with it. Left at the pre-reset total, the
       next flush would not fire until the walk had covered that distance
       AGAIN, and the stale total would sit in state until it did. */
    st.lastWritten = 0;
    st.dirty = true;
    if (streamId) st.streamId = streamId;
    flushState();
    renderDistance();
  ...
  addEventListener('onEventReceived', (e) => {
    ...
    /* Live channel-state frames, pushed by the server on every mutation.
       (The old 'stream.online' listener that sat here never fired: that
       event only reaches the rest-api webhook, not an overlay's room.) */
    else if (d.listener === 'streamwizard.user_state' && d.event) {
      const k = d.event.key, v = d.event.value;
      if (k === 'sys.stream_id') {
        /* Track the current stream live, so mid-walk flushes bank against
           the stream running NOW, not the one at page load. */
        st.streamId = v || null;
      } else if (k === 'distance' && txt(cfg.distanceReset, 'stream') === 'stream'
                 && v && Number(v.m) === 0 && st.total > 0
                 && !bool(cfg.demoMode, false)) {
        /* The server applied its reset-on-stream policy — adopt it. Guarded
           by st.total > 0 so the echo of our own m=0 flush can't loop; the
           calibration factor survives, as always. */
        resetDistance(st.streamId);
      }
    }
  });
...
$widgetjs$,
  ...
  UPDATE public.overlay_widgets w
  SET js = t.js, updated_at = now()
  FROM public.overlay_widget_templates t
  WHERE t.slug = 'walking-stats' AND md5(w.js) = v_prev_md5;
  GET DIAGNOSTICS v_snapshots = ROW_COUNT;

  INSERT INTO public.user_state_definitions (user_id, key, reset_policy, reset_value, reset_grace_seconds)
  SELECT DISTINCT w.user_id, 'distance', 'stream', '{"m": 0}'::jsonb, 900
  FROM public.overlay_widgets w
  JOIN public.overlay_widget_templates t ON t.slug = 'walking-stats' AND w.js = t.js
  ON CONFLICT (user_id, key) DO NOTHING;
  GET DIAGNOSTICS v_definitions = ROW_COUNT;

  RAISE NOTICE 'walking-stats v5: % snapshot(s) updated, % definition(s) seeded', v_snapshots, v_definitions;
END $$;
