-- Walking Stats: fix ReferenceError introduced by the stationary-pin patch.
--
-- The pin block in addDistance() referenced `now`, which is local to
-- handleGeo(): every fix carrying a Doppler speed threw before any distance
-- could accumulate, so the bar sat at 0 while GPS publishing (a separate
-- path) kept logging fine. addDistance now derives the timestamp from the
-- fix itself, the same way calibrate() does.
--
-- Verified in a DOM-stubbed harness: 168 m walk shows 166, a 90 s standstill
-- with phantom 0.7 m/s Doppler adds 4 m and reads 0 km/h, replayed stale
-- echoes add nothing, zero uncaught errors.
