-- Walking Stats v4: the distance total moves to channel state.
--
-- The bug: the widget resets its distance when it receives stream.online, and
-- a widget only receives events while it is open. A streamer who goes live
-- before starting OBS never gets the reset, so the overlay restores the last
-- walk's kilometres and shows them as if they belonged to the new stream.
-- Reloading mid-stream had the same effect.
--
-- The fix inverts the question. The server records the current stream id in
-- user_states (sys.stream_id) whether or not anything is connected, and the
-- widget banks its total alongside the stream it belongs to. At load it
-- compares the two: same stream, restore; different stream, start at zero.
-- Nothing depends on catching an event any more.
--
--   * distance        { m, streamId, savedAt }  written every 10 m of travel
--   * gps_cal_factor  number                    written only when it changes
--
-- An UNKNOWN current stream restores rather than resets. Zeroing on "I don't
-- know" (bot down, no Twitch integration, editor preview) would wipe a walk in
-- progress on every reload, which is worse than the bug being fixed.
--
-- Flushing on distance rather than on a timer means a parked phone writes
-- nothing at all; a walk writes about once every 7 s. pagehide and
-- visibilitychange flush the last stretch so closing OBS doesn't drop up to 10 m.
--
-- The stream.online listener stays: it is idempotent and resets instantly for
-- an overlay that did happen to be open, instead of at the next load.
--
...
-- template linkage, so template-only updates leave every existing user on the
-- old code.
...
DECLARE
  ...
  prev_js_md5    text;
BEGIN
  -- Captured before the update so this migration carries its own predecessor
  -- rather than a hardcoded hash that has to be remembered each time.
  SELECT md5(js) INTO prev_js_md5
  FROM public.overlay_widget_templates
...
(() => {
  ...
  async function loadState() {
    ...
    try {
      ...
      /* Only a KNOWN and DIFFERENT stream clears the total. An unknown
         current stream — bot down, no Twitch integration, editor preview —
         must restore: zeroing on "I don't know" would wipe a walk in
         progress on every reload, which is worse than the missed reset this
         replaces. */
      if (mode === 'stream' && current && banked.streamId !== current) {
        /* Bank the zero against the new stream straight away, so a second
           reload before the first 10 m doesn't have to re-derive it. */
        flushState();
        return;
      }
  ...
  addEventListener('onEventReceived', (e) => {
    ...
    /* Kept even though loadState now converges on its own: it costs nothing,
       it is idempotent, and it resets instantly for an overlay that happened
       to be open rather than at the next load. */
    else if (d.listener === 'stream.online' && txt(cfg.distanceReset, 'stream') === 'stream') {
      resetDistance(d.event && d.event.id);
    }
  });
