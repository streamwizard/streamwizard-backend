-- Walking Stats widget: distance-accounting fixes.
--
-- 1. Duplicate/stale frame guard in handleGeo. On the phone (gps render mode)
--    each fix used to reach the widget twice: once from local context and once
--    as the ws-server echo of the page's own publish. In-order duplicates were
--    harmless (dt = 0), but a lagging echo rewound st.lastFix (assigned outside
--    the dt guard) and the next fix re-counted the same interval. A 68-minute
--    test walk of 4.4 km displayed 6.4 km this way. The frontend now also drops
--    the echo (CustomWidgetIframe), but the widget must not depend on that.
-- 2. Demo motion no longer persists: startAutoSave skips while demoMode is on,
--    so synthetic travel cannot bank km that a later real stream starts from.
-- 3. Calibration retune: target weighted toward the position-path odometer
--    (map-matched walks show 5s strides within ~2% of truth; Doppler reads
--    15-20% low) and the factor ceiling raised 1.15 -> 1.25 so the learned
--    correction can actually reach the observed bias.
-- 4. Stationary pin + Doppler accuracy gate. Indoors the chip reports phantom
--    Doppler speed (0.3-0.8 m/s) while the position never leaves a small
--    circle, which integrated to ~30 m/min at a standstill. Speed-based
--    distance now only accumulates while the position keeps escaping an
--    accuracy-sized pin circle, the displayed speed reads 0 while pinned, and
--    fixes rejected by the accuracy gate no longer feed Doppler integration.

UPDATE public.overlay_widget_templates
SET js = '/* IRL bottom bar — speed / distance / location / weather */
(() => {
  ''use strict'';
  ...
  function addDistance(g, dt, accOk) {
    ...
    if (sp !== null) {
      ...
      if (!accOk) { st.prevSp = sp; st.anchor = null; return; }
      ...
      if (!st.pin) st.pin = { lat: g.latitude, lon: g.longitude, acc: pinAcc, t: now };
  ...
  /* Runs two odometers side by side and steers a correction factor toward
     ...
     Every 30 raw Doppler metres, compare the two windows:
       ...
       target = 0.25 + 0.75 * ratio  — weighted toward the position path.
                Map-matching real walks showed 5s stride hops track true
                distance within ~2%, while Doppler reads 15-20% low, so the
                position odometer deserves most of the weight.
  ...
  function calibrate(g, accOk) {
    ...
    if (!accOk || !movingNow || st.pinStill) { c.stride = null; return; }
    ...
    if (c.dop >= 30 && c.pos > 0) {
      ...
      if (ratio > 0.7 && ratio < 1.6) {
        const target = clamp(0.25 + 0.75 * ratio, 0.9, 1.25);
  ...
  function handleGeo(raw) {
    ...
    if (st.lastFix && now <= st.lastFix.t) return;
